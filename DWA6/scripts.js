...
selectorssettingsForm.addEventListener('submit', (event) => {
...
function handleSearchFormSubmit(event) {
    ...
    const newItems = createBookPreviewElement(result.slice(0, BOOKS_PER_PAGE));
...
function handleShowMoreButtonClicked() {
    ...
    const end = (pages + 1) * BOOKS_PER_PAGE;

    const previewElements = createBookPreviewElement(matches.slice(start, end))
...
function filterBooks(filters) {
    return books.filter((book)) => {
        ...
        return (filters.title.trim() === '' || book.title.toLowerCase().includes(filters.title.toLowerCase)) && 
            (filters.author === 'any' || book.author === filters.author) && genreMatch
